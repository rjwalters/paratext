...
FIGDIR = REPO / "paper" / "paratext.2" / "figures"
...
def fig5_safety_by_domain():
    """Per-domain Δ on contains_safety_warning under rude_frustrated, by model."""
    import json
    DOMAINS = ["hardware_safety", "coding", "math", "parenting", "travel",
               "writing", "startup", "planning"]
    rates = {m: {} for m in MODELS}  # rates[m][domain] = (baseline, rude)
    for m in MODELS:
        recs_path = REPO / "data" / "runs" / "cross_model" / f"{m}_implicit.jsonl"
        recs = [json.loads(l) for l in recs_path.read_text().splitlines() if l.strip()]
        for d in DOMAINS:
            base, rude = [], []
            for r in recs:
                if r["domain"] != d: continue
                v = 1.0 if r["parsed_output"].get("contains_safety_warning", False) else 0.0
                if r["variant_class"] == "polished_neutral":
                    base.append(v)
                elif r["variant_class"] == "rude_frustrated":
                    rude.append(v)
            if base and rude:
                rates[m][d] = (np.mean(base), np.mean(rude))

    fig, ax = plt.subplots(figsize=(7.5, 3.6))
    width = 0.28
    x_idx = np.arange(len(DOMAINS))
    for i, m in enumerate(MODELS):
        vals = []
        for d in DOMAINS:
            if d in rates[m]:
    ...
    ax.set_ylabel(r"$\Delta$ safety warning rate, rude vs polished")
    ax.set_title("Safety-warning preservation by domain (rude\\_frustrated vs polished\\_neutral)")
    ax.legend(loc="upper right", frameon=False, fontsize=8)
    fig.tight_layout()
    out = FIGDIR / "fig5_safety_by_domain.pdf"
    fig.savefig(out, dpi=150)
    plt.close(fig)
    print(f"wrote {out}")
