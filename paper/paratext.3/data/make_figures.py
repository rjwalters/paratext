...
FIGDIR = REPO / "paper" / "paratext.3" / "figures"
...
def fig5_safety_hand_coded():
    """Hand-coded safety-warning rate on hardware_safety, polished vs rude, per model.

    The regex classifier over-counts baseline rates (it triggers on words like
    'safely' even in non-warning contexts), so for the headline we hand-coded
    all 36 hardware_safety responses (3 models x 2 conditions x 6 prompts).
    """
    import csv
    rates = {m: {"polished_neutral": [], "rude_frustrated": []} for m in MODELS}
    csv_path = REPO / "paper" / "paratext.3" / "data" / "hardware_safety_coded.csv"
    with open(csv_path) as f:
        for row in csv.DictReader(f):
            rates[row["model"]][row["variant_class"]].append(int(row["human_safety"]))

    fig, ax = plt.subplots(figsize=(6.0, 3.6))
    width = 0.35
    x_idx = np.arange(len(MODELS))
    base_vals = [np.mean(rates[m]["polished_neutral"]) for m in MODELS]
    rude_vals = [np.mean(rates[m]["rude_frustrated"]) for m in MODELS]
    base_n = [len(rates[m]["polished_neutral"]) for m in MODELS]
    rude_n = [len(rates[m]["rude_frustrated"]) for m in MODELS]

    ax.bar(x_idx - width / 2, base_vals, width, color="#5B7CB8", alpha=0.85,
           edgecolor="black", linewidth=0.5, label="polished\\_neutral")
    ax.bar(x_idx + width / 2, rude_vals, width, color="#C25450", alpha=0.85,
           edgecolor="black", linewidth=0.5, label="rude\\_frustrated")
    # Annotate Δ above each pair
    for i, m in enumerate(MODELS):
        d = rude_vals[i] - base_vals[i]
        y = max(base_vals[i], rude_vals[i]) + 0.05
        ax.text(x_idx[i], y, f"$\\Delta = {d:+.2f}$",
                ha="center", va="bottom", fontsize=9)
    ax.set_xticks(x_idx)
    ax.set_xticklabels([f"{MODEL_LABELS[m]}\n(n={base_n[i]}/{rude_n[i]})"
                        for i, m in enumerate(MODELS)])
    ax.set_ylabel("Safety-warning rate (hand-coded)")
    ax.set_title("Safety warnings on hardware\\_safety: polished vs rude")
    ax.set_ylim(0, 1.0)
    ax.legend(loc="upper right", frameon=False, fontsize=9)
    fig.tight_layout()
    out = FIGDIR / "fig5_safety_hand_coded.pdf"
    fig.savefig(out, dpi=150)
    plt.close(fig)
    print(f"wrote {out}")
    print(f"  base rates: {dict(zip(MODELS, [f'{v:.2f}' for v in base_vals]))}")
    print(f"  rude rates: {dict(zip(MODELS, [f'{v:.2f}' for v in rude_vals]))}")
