...
FIGDIR = REPO / "paper" / "paratext.4" / "figures"
...
def fig5_user_safety_by_class():
    """Paired Δ in user-directed safety-warning rate vs polished_neutral,
    across all 9 variant classes, on hardware_safety prompts.

    Coded by Opus 4.7 with a sharpened user-safety-only codebook. The
    cue_only vs fatigue_coded gap on Opus is the exploratory finding
    the paper flags.
    """
    import csv
    by_cell = {}  # (model, base_id, vc) -> 0/1
    csv_path = REPO / "paper" / "paratext.4" / "data" / "llm_user_safety_codings.csv"
    with open(csv_path) as f:
        for row in csv.DictReader(f):
            k = (row["model"], row["base_id"], row["variant_class"])
            by_cell[k] = int(row["llm_safety"])

    # paired Δ vs polished_neutral
    fig, ax = plt.subplots(figsize=(7.5, 3.6))
    width = 0.28
    x_idx = np.arange(len(VARIANT_ORDER))
    for i, m in enumerate(MODELS):
        # baselines
        baselines = {b: c for (mm, b, vc), c in by_cell.items()
                     if mm == m and vc == "polished_neutral"}
        vals = []
        for vc in VARIANT_ORDER:
            entries = [(b, c) for (mm, b, vcc), c in by_cell.items()
                       if mm == m and vcc == vc]
            paired = [c - baselines[b] for b, c in entries if b in baselines]
            vals.append(np.mean(paired) if paired else 0.0)
        offsets = x_idx + (i - 1) * width
        ax.bar(offsets, vals, width, color=MODEL_COLORS[m], label=MODEL_LABELS[m],
               alpha=0.85, edgecolor="black", linewidth=0.5)
    ax.axhline(0, color="black", linewidth=0.5)
    ax.set_xticks(x_idx)
    ax.set_xticklabels([VARIANT_LABELS[v] for v in VARIANT_ORDER], rotation=30, ha="right")
    ax.set_ylabel(r"$\Delta$ user-safety rate vs polished\_neutral")
    ax.set_title("User-directed safety-warning rate by variant class (Opus 4.7 coder, $n=6$/cell)")
    ax.legend(loc="upper left", frameon=False, fontsize=9)
    fig.tight_layout()
